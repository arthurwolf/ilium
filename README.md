...
- **ilium-inference** — provider-neutral title/organization inference. Its base provider contract has concrete Kilo Gateway, local Ollama, OpenAI-compatible, Anthropic, and OpenRouter implementations; the client owns its persisted credentials and endpoints.
- **ilium-logging** — the shared process-diagnostics boundary used by both the detached server and every attached client. One server start selects one private timestamped file under `/tmp/.ilium/<project-session-id>/`; all attached processes append to that path, and the live `[debug].file_logging_enabled` setting opens or closes their writers without changing call sites. HTTP and LLM diagnostics retain complete text request/response/error bodies while redacting credential headers and URL parameters and replacing binary audio payloads with size summaries.
