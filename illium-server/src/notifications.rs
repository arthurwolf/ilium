...
use illium_core::{AgentActivity, PaneStatus};
...
/// was `Agent(_, Working)` and `new` is `Agent(_, Idle)` or
...
/// there is no prior `Working` turn that just ended; `Idle -> Working` (a
/// turn starting) never notifies, since nothing finished; and
/// `Working -> WaitingApproval` never notifies either -- the agent is
/// blocked on the user, not done, and `illium_detect::classify_activity`
/// already distinguishes that case from "finished" precisely so callers
/// like this one don't have to guess.
pub fn is_finished_transition(previous: Option<&PaneStatus>, new: &PaneStatus) -> bool {
    let Some(previous) = previous else {
        return false;
    };
    matches!(previous, PaneStatus::Agent(_, AgentActivity::Working))
        && matches!(
            new,
