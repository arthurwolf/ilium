...
/// video) -- ilium never calls `TextArea::set_cursor_style`, so this is
...
fn render_row_spans(
    ...
    // applicable style via `patch` in the same priority order the per-byte
    // cells above use, instead of picking only one.
    if cursor_at_row_end || select_trailing {
...
mod tests {
    ...
    fn scratch_dir() -> std::path::PathBuf {
        let dir = std::env::temp_dir()
            .join("ilium-editor-highlight-tests")
