...
/// Hard cap on distinct glyph entries `swash_cache` may accumulate before
/// `rasterize` resets it. `cosmic_text::SwashCache` caches every rasterized
/// glyph image (and outline) it ever produces, keyed by (font, size, glyph
/// id, subpixel bin) with no eviction of its own. Since `HeaderRasterizer`
/// is built once and reused for every header in every document opened for
/// the life of the client process (see `App::markdown_rasterizer`), that
/// cache would otherwise grow without bound over a long session as new
/// documents, header tiers, and terminal cell sizes (resize) are seen.
/// Re-rasterizing after a reset costs only a few milliseconds -- this
/// cache exists purely to dedupe repeat draws within/across nearby frames
...
impl HeaderRasterizer {
    ...
    pub fn rasterize(
        ...
        // See `MAX_SWASH_CACHE_ENTRIES`'s doc comment: reset the glyph
        // cache before it grows past the cap rather than letting it
        // accumulate for the rest of the process's life.
        if self.swash_cache.image_cache.len() >= MAX_SWASH_CACHE_ENTRIES
            || self.swash_cache.outline_command_cache.len() >= MAX_SWASH_CACHE_ENTRIES
        {
...
/// Needed because adjacent glyphs' anti-aliased edges can land on the same
/// output pixel (italic slants, kerning, ligatures), so `dst` isn't always
/// fully transparent or fully opaque when a second draw call hits it -- the
/// weighting has to account for `dst`'s own alpha, not just `src`'s.
...
mod tests {
    ...
    fn rasterizes_nonempty_header_to_nonzero_image() {
        ...
        // Dimensions alone are guaranteed by the `.max(1.0)` clamps even if
        // shaping/drawing produced nothing -- also check that some pixel
        // actually got painted, i.e. glyphs were really rasterized.
        assert!(
            image.pixels().any(|pixel| pixel.0[3] > 0),
            "expected at least one non-transparent pixel from rasterizing \"Hello\""
        );
    }

    /// Returns the vertical span (in pixels) of non-transparent ink in the
    /// image, used below as a proxy for the rasterized glyph's font size
    /// since the canvas height itself is fixed at two text rows regardless
    /// of heading tier.
    fn ink_height(image: &RgbaImage) -> u32 {
        let (min_y, max_y) =
            image
                .enumerate_pixels()
                .fold((u32::MAX, 0u32), |(min_y, max_y), (_, y, pixel)| {
                    if pixel.0[3] > 0 {
    ...
    fn smaller_tiers_produce_smaller_font_size() {
        ...
        // Both share the same fixed 2-row pixel canvas height; the visual
        // size difference is in glyph scale, so measure the actual ink
        // (drawn glyph pixels) rather than the canvas dimensions.
        assert_eq!(h1.height(), h6.height());
        assert!(
            ink_height(&h1) > ink_height(&h6),
            "expected level-1 header ink ({}) to be taller than level-6 ({})",
            ink_height(&h1),
            ink_height(&h6)
        );
    }
