...
impl Row {
    ...
    pub fn erase(&mut self, i: u16, attrs: crate::attrs::Attrs) {
        ...
        // `wide` is captured before erasing because a two-cell character's
        // logical final column starts at `cols() - 2`. Saturation also keeps
        // an already-orphaned wide cell in a one-column row from underflowing.
    ...
    pub fn resize(&mut self, len: u16, cell: crate::Cell) {
        ...
        self.wrapped = false;
        // A shrinking resize can remove a wide character's continuation while
        // retaining its leading cell at the new row edge. Clear that orphan so
        // later erase operations never attempt to reach beyond the row.
    ...
    pub fn clear_wide(&mut self, col: u16) {
        ...
        let other = if cell.is_wide() {
            ...
            if next >= self.cells.len() {
                // Defend against malformed or legacy state containing a wide
                // leading cell whose continuation no longer exists.
...
#[cfg(test)]
mod ilium_regression_tests {
    use super::*;

    #[test]
    fn erase_orphaned_wide_cell_in_one_column_row_does_not_underflow() {
        let attrs = crate::attrs::Attrs::default();
        let mut row = Row::new(1);
        row.get_mut(0)
            .expect("one-column row should contain its first cell")
            .set('中', attrs);

        row.erase(0, attrs);

        let cell = row.get(0).expect("erasing must retain the row cell");
