...
//! process-tree walk -- so it is genuinely client-local work: illium-client
...
//! exist in the legacy `illium` bin crate's `agent_detect.rs`
...
//! illium-server now owns every pane's PTY), so they can't simply move
...
//! rewires the `illium` bin to stop owning detection itself, at which
...
mod tests {
    ...
    fn scratch_dir() -> PathBuf {
        let dir = std::env::temp_dir()
            .join("illium-client-session-transcript-tests")
