...
mod tests {
    ...
    fn scratch_dir() -> PathBuf {
        let path = std::env::temp_dir()
            .join("illium-project-naming-tests")
    ...
    fn prompt_is_xml_shaped_and_includes_the_json_output_example() {
        ...
        assert!(prompt.contains("<output-example>{\"project_name\":\"Illium\"}</output-example>"));
    }
    ...
    fn rejects_non_json_and_more_than_two_words() {
        assert!(parse_project_name_response("Illium").is_err());
