//! Modal file-picker overlay, opened when the user creates a new editor
//! pane. Owns its own directory listing and renders it as a bordered
...
impl ExplorerOverlay {
    ...
    /// Re-lists `current_dir` and resets the selection to the top: `..`
    /// first when it has a parent, then directories, then files, each
    /// group sorted alphabetically case-insensitively. A single unreadable
    /// entry (permission race, vanished mid-scan) is skipped rather than
    ...
    fn handle_key(&mut self, key: &KeyEvent, screen_area: Rect) -> anyhow::Result<Option<PathBuf>> {
        ...
        match key.code {
            ...
            KeyCode::Right | KeyCode::Char('l') | KeyCode::Enter => return self.activate(),
            _ => {}
        }
        Ok(None)
    }
...
pub fn render(frame: &mut Frame, screen_area: Rect, overlay: &ExplorerOverlay, now: SystemTime) {
    ...
    let title = theme::chrome_title(&format!("Open File — {}", overlay.current_dir.display()));
    ...
    let hint = if overlay.entries.is_empty() {
        ...
        "↑↓/j/k move · →/Enter/l open · ←/⌫/h up · Ctrl+H hidden files · Esc cancel"
    };
    frame.render_widget(
...
mod tests {
    ...
    fn scratch_dir(label: &str) -> PathBuf {
        let dir = std::env::temp_dir()
            .join("illium-explorer-overlay-tests")
