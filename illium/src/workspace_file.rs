//! On-disk workspace snapshot: `.illium/sessions.yml` under the session's
//! `--cwd`. Lets a killed/closed illium be reopened in the same folder and
...
const RELATIVE_PATH: &str = ".illium/sessions.yml";
...
pub struct WorkspaceFile {
    ...
    /// the root (see `illium_core::Tree::add_pane`'s own invariant), but
...
pub struct SavedAgent {
    ...
    pub command: String,
    /// Known only once one of illium's detection tiers resolved it; a
    /// pane saved before that happened restores as a fresh (non-resumed)
    /// agent invocation instead.
    pub session_id: Option<String>,
}

/// Writes `workspace` to `<cwd>/.illium/sessions.yml`, creating the
/// `.illium` directory if needed. Writes to a temp file in the same
...
/// Reads and parses `<cwd>/.illium/sessions.yml`. `Ok(None)` means no save
...
mod tests {
    ...
    fn scratch_dir() -> PathBuf {
        let dir = std::env::temp_dir()
            .join("illium-workspace-file-tests")
            ...
            children: vec![
                ...
                SavedNode::Editor {
                    ...
                    path: Some(PathBuf::from(
                        "/home/developer/dev/ai/illium/illium/src/main.rs",
                    )),
                },
                SavedNode::Group {
    ...
    fn save_overwrites_a_previous_snapshot_atomically() {
        ...
        let leftovers: Vec<_> = std::fs::read_dir(cwd.join(".illium"))
            .unwrap()
