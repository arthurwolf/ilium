...
//! in illium reads as one visual language instead of three.
...
pub fn render_text_prompt(
    ...
    // illium's prompts only ever hold pane/group names and shell command
    ...
    let cursor_x = rows[0].x + u16::try_from(state.cursor).unwrap_or(u16::MAX);
    frame.set_cursor_position(Position::new(cursor_x.min(rows[0].right()), rows[0].y));
}
