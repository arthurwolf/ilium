...
//!
//! The rest of Illium depends only on `KiloGatewayClient::complete_text`:
...
/// The no-credential model selected for Illium's lightweight metadata jobs.
...
impl KiloGatewayClient {
    ...
    /// `kilo-auto/free` calls by public IP, which is the requested default.
    ...
    fn retries_temporary_failures_then_returns_assistant_text() {
        ...
        let result = client.complete_text_with_sender(&request(), |_| {
            ...
            Ok(ChatCompletionResponse {
                choices: vec![ChatChoice {
                    message: ChatChoiceMessage {
                        content: Some("Illium".to_string()),
                    },
                }],
            })
        });

        assert_eq!(result.unwrap(), "Illium");
