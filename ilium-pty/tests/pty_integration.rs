//! Integration tests that spawn a real pty and a trivial real command. This
//! crate's whole job is talking to the OS, so faking that away would test
//! nothing real -- these are deliberately not unit tests with a mocked pty.
//!
//! Most of the file is Unix-only because it is built on `cat`'s exact
//! stdin-echo semantics and on `sh` scripts, neither of which Windows has.
//! `spawned_command_output_appears_on_screen` deliberately is not: it is the
//! one test that proves ConPTY spawns a process, renders its output, and
//! terminates, which is the claim that matters most on Windows.
...
/// Prints one known line and exits, spelled per platform: `echo` is a Unix
/// program, while on Windows it is a `cmd` builtin rather than an executable.
#[cfg(unix)]
fn echo_command(message: &str) -> PtyCommand {
    PtyCommand::new("echo", std::env::temp_dir(), 24, 80).arg(message)
}

#[cfg(windows)]
fn echo_command(message: &str) -> PtyCommand {
    PtyCommand::new("cmd", std::env::temp_dir(), 24, 80)
        .arg("/C")
        .arg(format!("echo {message}"))
}

#[test]
fn spawned_command_output_appears_on_screen() {
    let command = echo_command("hello from ilium-pty");
    let session = PtySession::spawn(command).expect("spawning the echo command should succeed");
    ...
    assert!(
        saw_output,
        "expected the command's output on screen, got: {:?}",
        session.screen_text()
    );
}

/// Everything below depends on `cat` echoing stdin verbatim, or on `sh`
/// scripts. Windows has neither, and substituting an approximation would test
/// the substitute rather than the pty. Tracked in docs/TODO.md.
#[cfg(unix)]
mod unix_only {
    use super::*;
    ...
    fn resize_truncating_wide_character_keeps_reader_parser_usable() {
        ...
        let session =
            PtySession::spawn(command).expect("spawning the resize fixture should succeed");
