//! Translates `ilium_ipc`'s wire-shaped mouse event types into
//! `crossterm::event::MouseEvent`, the type `ilium_pty::PtySession::write_mouse_input`
//! expects. Kept separate from `ilium-pty`'s own `mouse` module -- that
...
/// from one `ilium_ipc::ClientRequest::MouseInput`'s fields.
